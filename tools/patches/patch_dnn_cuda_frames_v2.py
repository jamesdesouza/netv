...
Version 2: Uses correct FFmpeg internal API (FilterLink/ff_filter_link)
...
# 4. Replace config_input to handle CUDA frames (using FilterLink API)
...
new_config_input = '''static int config_input(AVFilterLink *inlink)
{
    ...
    if (inlink->format == AV_PIX_FMT_CUDA) {
        ...
        hw_frames = (AVHWFramesContext *)il->hw_frames_ctx->data;
...
# 5. Replace config_output to handle CUDA output frames (using FilterLink API)
